...
GET_WEATHER_FUNCTION_DESC = {
    ...
    "function": {
        ...
        "description": (
            ...
            "用户未指定地点时必须调用本工具（默认查亳州），绝不能因为不知道地点就不调。"
            "用户未指明具体城市时，使用 default_location 查询天气。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
def parse_weather_info(soup):
    # 城市名
    city_name = soup.select_one("h1") or soup.select_one("title")
    city_name = city_name.get_text(strip=True) if city_name else "未知"

    # 当前天气摘要
    current_abstract = soup.select_one(".current-abstract")
    current_abstract = (
    ...
    for row in soup.select(".city-forecast-tabs__row")[:7]:
        date_el = row.select_one(".date-bg .date") or row.select_one(".date")
        date = date_el.get_text(strip=True) if date_el else "?"
        icon_el = row.select_one(".date-bg .icon") or row.select_one(".icon")
        weather_code = "999"
        if icon_el and icon_el.get("src"):
            weather_code = icon_el["src"].split("/")[-1].split(".")[0]
        weather = WEATHER_CODE_MAP.get(weather_code, "未知")
        temps = [span.get_text(strip=True) for span in row.select(".temp")]
        high_temp, low_temp = (temps[0], temps[-1]) if len(temps) >= 2 else (None, None)
        temps_list.append((date, weather, high_temp, low_temp))

    return city_name, current_abstract, temps_list
...
def get_weather(conn: "ConnectionHandler", location: str = None, lang: str = "zh_CN"):
    ...
    default_location = weather_config.get("default_location", "亳州")
    ...
    city_name, current_abstract, temps_list = parse_weather_info(soup)
